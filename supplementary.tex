...
\section{Supplementary Note 3: Constructing bridges in real-time}\label{supp:note3}
A bridge has 4 levels of completion, ranging from 1 to 4 depends on how close to finish it is.
When a new bridge is built with only one unique ending, it has level 1.
If both endings are identified, its completion level is 2.
...
Finally, when the only path solution is determined out of all candidates, the bridge is assigned as completely built with level 4.

In the early stages (level 1 or 2), a bridge is constructed progressively by alignments from long reads that spanning its corresponding anchor(s) (figure \ref{figure:npgraph_merging}).

\begin{figure}[!hpt]
\centering
\includegraphics[width=.8\textwidth]{images/bridge_merging.pdf}
\caption[Example of bridge merging progressively]{Bridge merging progressively in real-time. Long reads generate alignments to the contigs where new bridges are created respectively. Bridges sharing same anchors can be merged together to form the ultimate, more comprehensive bridge (with more spans and better approximation about distances between them).}
\label{figure:npgraph_merging}
\end{figure}

\section{Supplementary Note 4: Path finding algorithm}\label{supp:note4}
...
Algorithm~\ref{algo:findpath} demonstrates the path finding module in general. In which, function 
...
On the other hand, if external binning algorithm is employed, the resulting output must be converted into a text file that specifies the corresponding bin of every contigs. By that, each line of the file would be:
$
\mathtt{<contig\_ID>} \; ~ \mathtt{<bin\_ID>}
$
where $\mathtt{bin\_ID}=0$ indicates unspecified binned contigs.
\section{Supplementary Note 5: Graph decomposing for output report}\label{supp:note5}
