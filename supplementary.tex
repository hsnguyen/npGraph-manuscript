...
%In general, the algorithm works well for isolate or simple mock metagenomics, but not robust enough for complicated community.
%
...
%However, due to false alignments from shorter contigs to the long reads, not all of the reported step nodes are neccessary to be appeared in the ultimate path resolved by the bridge. 
...
%
%\section{Supplementary Note 6: Output the results}\label{supp:note6}
%The resolved assembly graph can be reported as a whole (GFA) or sequences only (FASTA).
%
%For the latter, longest-possible linear straight paths must be extracted from the graph for the final FASTA records. 
...
%\end{figure}
%
%Supplementary Figure~\ref{figure:npgraph_graphview} presents an example of the results before and after graph resolving process in the GUI.
...
\caption{Benchmarking \npgraph{} against \npscarf{}, $\mathtt{hybridSPAdes}$ and \unicycler{} hybrid assembler with the synthetic data set.}
...
\begin{figure}[!hpt]
\centering
\subfloat[ZymoEven assembly graph from $\mathtt{metaSPAdes}$]{
	\includegraphics[width=.45\textwidth]{images/zymoeven_ag.png}
}
~
\subfloat[ZymoLog assembly graph from $\mathtt{metaSPAdes}$]{
	\includegraphics[width=.45\textwidth]{images/zymolog_ag.png}
}
\caption{}
\label{supp_fig:npgraph_zymo}
\end{figure}
\end{document}
