...
The applications on synthetic and real data set of isolate genomes show improved accuracy while still maintain the required computational cost at very low level. In addition, the GUI feature can provide users a comprehensive look-and-feel of the whole assembly process. The tool and source code is available at \url{https://github.com/hsnguyen/assembly}.
%=======
%A real-time assembly pipeline for Oxford Nanopore Technology (ONT) data is important for either better control of sequencing resources or quick turnaround of analyses. The previous approach from \npscarf{} provided a greedy fast-response, auto-correct streaming algorithm for such task but was relatively prone to mis-assemblies. Here we present another real-time hybrid assembler, \npgraph{}, that can work on the assembly graph instead of the pre-assembled contigs. The results show improved accuracy while still minimize the required computational cost. The tool is available at \url{https://github.com/hsnguyen/assembly}.
%>>>>>>> 2d4c481dd93902dfb2dabe46534a62e8a8c2058c
}
...
\subsection*{Metagenomics assembly}
...
\begin{table}[!hpt]
\begin{center}
\caption{NGA50 improvement from $\mathtt{metaSPAdes}$ to \npgraph{} assembly on two mock communities ZymoLog and ZymoEven. The statistics were generated by $\mathtt{metaQUAST}$ v3.2 with 8 (out of 10) available PacBio references of individual isolates \cite{Nick2019zymo}.}
\label{table:zymo}
...
Even though the total amount of base coverage from the latter was almost four-times richer than the former, its $\mathtt{metaSPAdes}$ assembly quality was lesser. This due to the fact of under-presented microbial cells from rare species in the log population that require much more input to compensate, resulting in significant smaller size of assembly contigs and more fragmented assembly graph (Supplementary Figure~\ref{supp_fig:npgraph_zymo}).

Table~\ref{table:zymo} shows the assembly improvement in terms of NGA50 after running \npgraph{} on the $\mathtt{metaSPAdes}$ initial graph.  
...
\npgraph{} had been able to resolve the assembly graph efficiently for Even community, with the maximum NGA50 improvement for \emph{S.~enterica} from $33$ Kbp to $4.2$ Mbp. 
On the other hand, genomes from the Log community witnessed increasing length of aligned contigs as well, but not as huge as the other. The longest NGA50 contigs belongs to \emph{P.aeruginosa} of 445 Kbp, however, the counterpart from the Even community was 4-fold than that, at 1.75 Mbp.
The only exception was \emph{B.~subtilis}, where \npgraph{} can only resolved 382 Kbp correct length from the zymo even while the respective figure was 433 Kbp for the zymo log. It is noteworthy that for this case, initially the $\mathtt{metaSPAdes}$ assembly for the former was slightly better compared to the latter.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Conclusions}
Assembly graph is the data structure describing the assembly components at  more details.
The current chapter brings this informative knowledge to the original \npscarf{}'s mechanism in two ways, either partly for the gap-filling module only (\npscarfg{}) or completely as the building block for the assembly construction method (\npgraph{}).
...
Furthermore, users can monitor the assembly in an interactive manner providing the assembly GUI.
...
\subsection*{Assembly graph resolving in real-time}
\npgraph{} resolves the graph by reducing its complexity perpetually using the long reads that could be generated in real-time.
