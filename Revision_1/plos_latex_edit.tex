...
\addnew{This information can aid the bridging phase later, as member contigs of a bridge are expected to appear in the same group. However, the criteria can be relaxed between closed groups given strong connecting evidence from long read alignments.}

The first step \addnew{in our binning procedure} is to cluster the big anchors (longer than $10$Kbp and in/out degree less than 2) based on their \emph{kmer} coverage.
To achieve this, we applied DBSCAN clustering algorithm \cite{Ester96adensity-based}, which utilises a customised metric function to map contigs into a one-dimensional space.  In order to 
 define a customised metric which is \cancel{sample} \addnew{simple} and fast to calculate, we assumed that a single long contig itself consists of a Poisson distribution of \emph{k-mers} count with the mean approximated by the contig's coverage. 
...
\cancel{Given a bridge with 2 anchors, a path finding algorithm is used to find all candidate paths between them. Each of these paths is given a score of alignment-based likelihood which are updated immediately as long as there is an appropriate long read being generated by the sequencer. As more nanopore data arrives, the divergence between candidates' score becomes greater and only the top-scored ones are kept for the next round.}
\cancel{We implement a modified stack-based version utilizing Dijkstra's shortest path finding algorithm to reduce the search space when using Depth-First Search.}

\cancel{Due to false alignments from shorter contigs to the long reads, not all of the reported step nodes are necessary to be appeared in the ultimate path resolved by the bridge. 
In most cases, the accumulated score of each step indicates its likelihood to be the true component of the final solution.
For that reason, a strategy similar to binary searching is employed to find a path across 2 anchors of a bridge as shown in Algorithm~2.
Before that, we define Algorithm~1 to demonstrates the path finding algorithm for two nodes given their estimated distance.}

\addnew{Given a proposed bridge with 2 anchors $B=\lbrace\overrightarrow{v_0}\,..., \overrightarrow{v_n}\rbrace$ identified from previous step, a searching algorithm is implemented to find the final bridging solution, \IE{} an ungapped path among all possiblities. To do so, each of the candidates is given a score of alignment-based likelihood which are updated immediately as long as there is an appropriate long read being generated by the sequencer. As more nanopore data arrives, the score divergence increases and only the highest one would be selected for the next round.}
...
\addnew{In Algorithm~\ref{algo:findpath}, we implement a modified stack-based Depth-First Search (DFS) using Dijkstra's shortest path finding algorithm~\cite{Dijkstra1959} to reduce the search space.}
In which, function 
\[
...
\cancel{A threshold for the traversing depth is set (150) to ignore too complicated and time-consuming path searching.}
\addnew{Another threshold for the maximum traversing depth is set and heuristic cut-off can be used to preven exceptional combinatorial explosion when traversing extremely complex graph components.}

\cancel{Note that the $length()$ functions for node and edge are totally different. While the former returns the length of the sequence represented by the node, \IE{} contig from short-read assembly, the latter is usually negative because an edge models a link between two nodes, which is normally an overlap (except for composite edges). For example, in a \emph{k-mers} SPAdes assembly graph, the value of an edge is $-k+1$.} 
\addnew{It worths noting that from the pseudo code in Algorithm~\ref{algo:findpath}, despite of identical name, procedure $length()$ used for nodes are different than for edges. The former would return the contig length while the latter measure the overlap or distance between a pair of contigs thus can be negative or positive respectively. From De Bruijin graph for instance, all edges have same the length value of $-k$. In \npgraph{}'s algorithm, an edge's length can vary as we allow additional connection types on top of the original \spades{} assembly graph input.}
...
To evaluate the performance of the method, \npgraph{} 1.1 was tested along with \spades{}, \spades{} hybrid from version 3.13.1, ~\cite{AntipovKM2015}, \npscarf{} (japsa 1.7-02), \addnew{\lrscaf{} version 1.1.9} and \unicycler{} version 0.4.6 using \unicycler{}`s synthetic data set~ \cite{Wick2017unicycler} . 
\addnew{}
...
\addnew{\lrscaf{} has been designed as a computationally efficient hybrid assembly tool which is scalable to large genomes.  We ran \lrscaf{} using default parameters on the extended benchmarking set presented in Supplementary table 1.  We observed that  \lrscaf{} assemblies had fewer misassemblies than \npscarf{}, and also exhibited a low mismatch and indel rate, but produced more fragmented assemblies than either \npgraph{} or \unicycler{}. }
...
\begin{figure}[!ht]
 ...
 \caption{Alignments of an \emph{Enterobacter~cloacae} reference genome to assembly sequences generated by  (a)~\cancel{\unicycler{}}\addnew{\npgraph{}} and (b)~\cancel{\npgraph{}}\addnew{\unicycler{}}. The former suggests a structural variant, the latter is virtually an 1-to-1 mapping.}
\end{figure}

\clearpage
\paragraph*{S1 Table.}
\label{S1_Table}
Benchmarking different methods using $\mathtt{\addnew{lrscaf}}$, \npscarf{}, \npgraph{}, $\mathtt{hybridSPAdes}$ and \unicycler{} hybrid assembler with the synthetic data set.
...
\nobreakmidrule
\rowcolor{Gray}
& \addnew{lrscaf} & \addnew{-} & \addnew{-} & \addnew{-} & \addnew{-} & \addnew{-} & \addnew{-} \\*
\rowcolor{Gray}
...
\nobreakmidrule
& \addnew{lrscaf} & \addnew{4113104} & \addnew{6} & \addnew{4002298} & \addnew{0} & \addnew{0.00} & \addnew{0.00}\\*
...
\nobreakmidrule
\rowcolor{Gray}
& \addnew{lrscaf} & \addnew{4119020} & \addnew{12} & \addnew{4002117} & \addnew{0} & \addnew{0.03} & \addnew{0.79}\\* 
\rowcolor{Gray}
...
\nobreakmidrule
& \addnew{lrscaf} & \addnew{3896799} & \addnew{4} & \addnew{3886381} & \addnew{0} & \addnew{0.47} & \addnew{
0.36}\\*
...
\nobreakmidrule
\rowcolor{Gray}
& \addnew{lrscaf} & \addnew{4367657} & \addnew{27} & \addnew{1035967} & \addnew{13} & \addnew{30.11} & \addnew{1.42}\\* 
\rowcolor{Gray}
...
\nobreakmidrule
& \addnew{lrscaf} & \addnew{4732001} & \addnew{16} & \addnew{692794} & \addnew{7} & \addnew{3.42} & \addnew{0.58}\\*
...
\nobreakmidrule
\rowcolor{Gray}
& \addnew{lrscaf} & \addnew{5410854} & \addnew{21} & \addnew{910949} & \addnew{9} & \addnew{13.70} & \addnew{0.87}\\*
\rowcolor{Gray}
...
\nobreakmidrule
& \addnew{lrscaf} & \addnew{5512518} & \addnew{20} & \addnew{777919} & \addnew{26} & \addnew{3.52} & \addnew{0.38}\\*
...
\nobreakmidrule
\rowcolor{Gray}
& \addnew{lrscaf} & \addnew{5823340} & \addnew{38} & \addnew{559414} & \addnew{7} & \addnew{5.92} & \addnew{1.27}\\*
\rowcolor{Gray}
...
\nobreakmidrule
& \addnew{lrscaf} & \addnew{5470282} & \addnew{8} & \addnew{2951488} & \addnew{9} & \addnew{3.04} & \addnew{2.62}\\*
...
\nobreakmidrule
\rowcolor{Gray}
& \addnew{lrscaf} & \addnew{4260869} & \addnew{12} & \addnew{3215170} & \addnew{8} & \addnew{6.69} & \addnew{2.27}\\*
\rowcolor{Gray}
...
\nobreakmidrule
& \addnew{lrscaf} & \addnew{12486884} & \addnew{214} & \addnew{254587} & \addnew{27} & \addnew{16.45} & \addnew{1.97}\\*
...
\nobreakmidrule
\rowcolor{Gray}
& \addnew{lrscaf} & \addnew{4678609} & \addnew{175} & \addnew{376897} & \addnew{26} & \addnew{3.91} & \addnew{2.35}\\*
\rowcolor{Gray}
...
\nobreakmidrule
& \addnew{lrscaf} & \addnew{5266645} & \addnew{128} & \addnew{429353} & \addnew{27} & \addnew{9.72} & \addnew{1.19}\\*
...
\nobreakmidrule
\rowcolor{Gray}
& \addnew{lrscaf} & \addnew{2231488} & \addnew{10} & \addnew{683404} & \addnew{6} & \addnew{11.96} & \addnew{1.37}\\*
\rowcolor{Gray}
