...
\addnew{In addition, the release of Read Until API enables direct control to the actual molecular reading process which has paved the way for customized targeted sequencing on ONT platforms. To function properly, such methods must implement a real-time ultrafast operation on the streaming data, \EG{} in ReadFish~\cite{Payne2020}. Up until now, there is a shortage of similar tools that can utilize this distinctive feature.}
...
Due to false \addnew{positive} alignments from shorter contigs to the long reads, not all of the reported step nodes \cancel{are necessary to be appeared} \addnew{would appear} in the ultimate path resolved by the bridge. 
In most cases, the accumulated score of each step indicates its likelihood to be the true component of the final solution.
For that reason, a strategy similar to binary search\cancel{ing} is employed to find a path across 2 anchors of a bridge as shown in Algorithm~\ref{algo:binsearch}. \addnew{Briefly, it recursively looks for sub-paths connecting pairs of step nodes in-between 2 anchors and return a final continuous path with the highest score.}
\cancel{Before that, }We define Algorithm~\ref{algo:findpath} \addnew{as the \emph{base} sub-rountine} to \cancel{demonstrates the path finding algorithm for} \addnew{find paths between a \emph{connectable} pair of} two \addnew{step} nodes given their estimated distance. In which, function 
\[
\mathtt{shortestTree}(\overrightarrow{vertex},distance) : (V,Z) \rightarrow V^n
\]
from line 3 \cancel{of the algorithm's pseudo code} builds a shortest tree rooted from $\overrightarrow{v}$, following its direction until a distance of approximately $d$ (with a tolerance \cancel{regarding nanopore read} \addnew{for the nanopore} error rate) is reached. This task is implemented based on Dijkstra algorithm.
